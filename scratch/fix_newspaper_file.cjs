...
import { FONT, getChocolateDate } from '../../pages/TravellersDesk';
...
export const GG_TravellerDeck_NewsPaper: React.FC<GG_TravellerDeck_NewsPaperProps> = ({
  ...
  const { coins, spendCoins, addCoins, addLegacy, user, latestFlashNews } = useTTStore();
  ...
  return (
    <div className="w-full h-full flex flex-col justify-between items-center select-none">
      <div className="w-full max-w-6xl flex-grow flex flex-col justify-between min-h-0">
        ...
        <div className="h-[90vh] mt-0 mb-0 bg-[#f7f2e8] border-[6px] border-amber-950/60 rounded-[2rem] p-6 text-amber-950 font-serif flex flex-col justify-between overflow-hidden shadow-2xl relative min-h-0 select-text">
          
          {/* Newspaper Top Header Section - Common for first page, simplified for others */}
          <div className="text-center border-b-4 border-double border-amber-950/40 pb-3 shrink-0 relative select-none">
...
console.log("Re-wrote newspaper pages correctly.");
