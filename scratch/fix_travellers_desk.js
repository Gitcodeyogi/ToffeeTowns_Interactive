...
// We want to search for the block starting from the first Floating Toggle handle (with animate-fade-in)
// down to the second Floating Toggle handle, and clean up the duplicate.
...
// Normalize line endings for replacement to avoid CR/LF mismatch issues
const normalize = str => str.replace(/\r\n/g, '\n').trim();

if (normalize(content).includes(normalize(targetBottomSearch))) {
  console.log('Found bottom target pattern!');
  // Let's do the replacement in a way that is robust to whitespace/newlines
  const idx = normalize(content).indexOf(normalize(targetBottomSearch));
  // Replace the target sequence directly
  // We can construct a regex or do a split/join if we match the normalized content
  // Since we want to be safe, let's write a splitter
