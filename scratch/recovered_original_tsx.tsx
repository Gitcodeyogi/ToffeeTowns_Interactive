...
const getCharacterReply = (char: CharacterTalk, text: string): string => {
  ...
  if (isVoteQuery) {
    ...
    const _dayContent = DAILY_ROTATION_DATA.find(d => d.day === dayIndex) || DAILY_ROTATION_DATA[0];
...
export const TownTalkModal: React.FC<TownTalkModalProps> = ({
  ...
  const { addCoins, addLegacy, coins, spendCoins } = useTTStore();
  ...
  useEffect(() => {
    const _unused = [getDailyDialogueText, dayContent, chocoDate];
    ...
    const timers = callers.map((caller, idx) => {
      ...
      return setTimeout(() => {
        setVisibleIds(prev => {
          ...
          if (prev.length > 0) {
            try { cozyAudio.playChime(); } catch(e) {}
          }
  ...
  const awardFriendshipPoints = (pts: number) => {
    ...
    if (leveledUp) {
      // Award coins
      addCoins(30, `Leveled up friendship with ${currentChar.name}`);
      addLegacy(15);
      ...
      addMessageToLog(selectedCharId, {
        sender: 'system',
        text: `❤️ Friendship Level Up! You are now Level ${nextLvl} with ${currentChar.name}. Gained +30 Cocoa Coins & +15 Legacy!`,
        time: timeStr
      });
